...
import { postCategoryImage } from "../../store/api/api";
...
const Admin = ({
  ...
  const uploadCategoryImageRef = useRef();

  const uploadCategoryImage = ({ target: { files } }, setValues, values) => {
    const temp = [];
              ...
              return (
                <AdminRow
                  items={[
                    ...
                    {
                      title:
                        deliveryWarehouse && deliveryCity
                          ? "Нова Пошта"
                          : "Самовивіз",
                      key: `${_id}delivery`,
                    },
          ...
          <TabPanel>
            <Formik
              ...
              onSubmit={async (
                ...
                if (
                  parentCategory?.parent?.length ||
                  parentCategory?.parentID
                ) {
                  categoryToSubmit.parentID =
                    parentCategory?.parent?._id || parentCategory?.parentID;
                  categoryToSubmit.subParentID = parentCategory._id;
                }
                let id = false;
                if (isCategoryEditing) {
                  id = await editCategory(
                    categoryToSubmit,
                    editing.category._id
                  );
                } else {
                  id = await createCategory(categoryToSubmit);
                }
                if (id) {
                  const formData = new FormData();
                  formData.append("gallery", gallery);
                  postCategoryImage(formData, id)
                    .then(() => {
                      resetForm({
                        name: "",
                        parentCategory: null,
                ...
                return (
                  <div>
                    <div className={s.add__category__container}>
                      <div className={s.add__category__main__content}>
                        <Input
                          name="name"
                        ...
                        <input
                          ...
                          style={{ display: "none" }}
                          onChange={(e) =>
                            uploadCategoryImage(e, setValues, values)
                          }
                          ref={uploadCategoryImageRef}
                        />

                        {!!values.image && (
                          <div className={s.category__image__container}>
                            <img
                              src={values.image}
                              alt="loading"
                        ...
                        <Button
                          ...
                          onClick={() =>
                            uploadCategoryImageRef?.current?.click()
                          }
                        />
                        {/*<Input*/}
                        {/*  name="gallery"*/}
                        {/*  label="Посилання на фото"*/}
                        {/*  placeholder="https://www.medtechnika.com.ua/media/wysiwyg/content_png/medicinskaja-tehnika.png"*/}
                        ...
                        return (
                          <AdminRow
                            ...
                            onEdit={() => {
                              setValues({
                                name: title,
                                parentCategory: sub[0] || parent[0],
                                image: gallery || "",
                                parentCategoryLabel:
                                  sub[0]?.title || parent[0]?.title,
                                title,
                              });
                              setEditing((prev) => ({
                                ...prev,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    editCategory: (category, id) => dispatch(editCategoryAction(category, id)),
